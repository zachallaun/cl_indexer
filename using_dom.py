...
class xml_parser():
	...
	def find_by_hood(self, neighborhood):
		...
		for x in items:
			# the three params that will be returned
			price = '' #empty string to store the price of each item
			bedrooms = -1 #int for the number of bedrooms
			url = x.link.string #storing so posts are not put into the database twice
			...
			if string.find(neighborhood) != -1 or string.find(neighborhood.lower()) != -1: #if the neighborhood string is found in the title
				...
				string = string[index:]
				for i in range(4): #loop adds pricing characters to 'price'
					price += (string[i + 1])
				...
				if bedrooms != -1:
					print '\n\n The original string: ', str(x.title)
